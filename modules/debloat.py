...
_LIST_DEBLOAT_PKG = './device/debloat_pkg.txt'
...
def rebloat():
	...
	# push script and list
	adb.push(os.path.abspath(_SCRIPT_DEBLOAT), _AOSPIFY_LOCAL)
	adb.push(os.path.abspath(_LIST_DEBLOAT_PKG), _AOSPIFY_LOCAL)
	...
	# run debloat script
	adb.shell('cd ' + _AOSPIFY_LOCAL + '; sh ' + _AOSPIFY_LOCAL + 'rebloat.sh')
